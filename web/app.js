...
window.xsmbFileResult=(message,error)=>toast(message,error);
async function download(raw,name,mime){
 const blob=new Blob([raw],{type:mime});
 if(window.XSMBFiles){
  if(blob.size>20000000)throw Error('File vượt 20 MB; hãy xuất khoảng thời gian ngắn hơn.');
  const encoded=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result.split(',')[1]);reader.onerror=()=>reject(Error('Không đọc được file xuất.'));reader.readAsDataURL(blob)});
  window.XSMBFiles.saveFile(encoded,name,mime);toast('Chọn thư mục và nhấn Lưu.');return;
 }
 ...
 toast('Đã gửi file tới trình duyệt.');
}
...
$('#import-csv').onclick=()=>attempt(async()=>{const file=$('#csv-file').files[0];if(!file)throw Error('Chọn file CSV trước.');if(file.size>4000000)throw Error('CSV phải nhỏ hơn 4 MB.');toast((await api('import',{text:await file.text()})).message);await refresh()});$('#csv-template').onclick=()=>attempt(()=>download('\ufeffday,numbers,special_prize\r\n','XSMB_mau_cot.csv','text/csv'));
...
$('#report-form').onsubmit=e=>{e.preventDefault();attempt(async()=>{const values={...formData($('#cau-form')),...formData($('#journal-form')),...formData(e.target),period:$('#finance-period').value,start:$('#finance-start').value,end:$('#finance-end').value};const res=await fetch('/api/export',{method:'POST',headers:{'Content-Type':'application/json','X-App-Token':token},body:JSON.stringify(values)});if(!res.ok)throw Error((await res.json()).error);await download(await res.arrayBuffer(),`XSMB_${values.kind}.${values.format}`,res.headers.get('Content-Type'))})};
$('#shutdown').onclick=()=>{if(confirm('Đóng ứng dụng XSMB?'))attempt(async()=>{if(window.XSMBFiles){window.XSMBFiles.closeApp();return}await api('shutdown',{});clearTimeout(pollTimer);document.body.innerHTML='<main style="margin:60px"><h1>Đã đóng XSMB V2 PRO</h1><p>Bạn có thể đóng tab này. Dữ liệu đã lưu vẫn được giữ.</p></main>'})};
